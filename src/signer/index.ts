...
import {RPC, Method, prepAndSign, publishRelays, fetchRelays, nip44, Status, makeRegisterResult, isRegisterRequest} from '../lib/index.js'
...
export class Signer {
  ...
  async handleRegisterRequest({payload}: RegisterRequest, event: TrustedEvent) {
    ...
    const share = tryCatch(() => PackageEncoder.share.deserialize(Buffer.from(payload.share, 'hex')))
    const group = tryCatch(() => PackageEncoder.group.deserialize(Buffer.from(payload.group, 'hex')))
    const cb = (status: Status, message: string) => channel.send(makeRegisterResult({status, message}))
    ...
    if (!Lib.is_group_member(group, share))        return cb(Status.Error, "Share does not belong to the provided group.")
    ...
    if (indices.size !== group.commits.length)     return cb(Status.Error, "Group contains duplicate member indices.")
    if (!commit)                                   return cb(Status.Error, "Share index not found in group commits.")
    if (commit.pubkey !== getPubkey(share.seckey)) return cb(Status.Error, "Share public key does not match group commit.")
    if (await this.sessions.has(event.pubkey))     return cb(Status.Error, "Client key has already been used.")
